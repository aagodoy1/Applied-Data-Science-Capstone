...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        pie_chart_data = spacex_df.groupby('Launch Site')['class'].value_counts().unstack(fill_value=0).reset_index()
        pie_chart_data['Total'] = pie_chart_data[1] + pie_chart_data[0]
        fig = px.pie(pie_chart_data, values='Total', names='Launch Site', title='Total Successful Launches by Site')
    else:
...
def update_scatter_chart(selected_site, payload_range):
    ...
    if selected_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', title=f'Correlation between Payload and Success at {selected_site}')
    return fig
# Run the app
if _name_ == '_main_':
    app.run_server()